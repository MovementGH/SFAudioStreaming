...
public:
    ...
    void Disconnect();
    template<typename Filter> void SetFilter(Filter* FilterPtr=nullptr) { delete m_Filter; if(FilterPtr==nullptr) m_Filter=new Filter(); else m_Filter=FilterPtr; }
    template<typename Codec> void SetCodec(Codec* CodecPtr=nullptr) { delete m_Codec; if(CodecPtr==nullptr) m_Codec=new Codec(); else m_Codec=CodecPtr; }
